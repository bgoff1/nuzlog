...
const getMatchups = async () => {
  const matchupList = await query(
    queryBuilder
      ...
      .whereRef("pokemon_v2_typeefficacy.damage_type_id", "=", "damage_type.id")
      .whereRef("pokemon_v2_typeefficacy.target_type_id", "=", "target_type.id")
      .compile(),
  );
...
export const useEffectiveness = () => {
  const [matchups] = createResource(getMatchups, { initialValue: new Map() });
